...
# Métodos propios interfaz

def generarRanking():
    print('Generando Ranking maiame')
    print(obtenerTipoResenia())
    print(obtenerFormatoReporte())


def validarFechas():
    fecha_desde = obtenerFechaDesde()
    fecha_hasta = obtenerFechaHasta()
    if fecha_desde > fecha_hasta:
        return False
    return True 


def obtenerFechaDesde():
    return date_entry_desde.get_date()

def obtenerFechaHasta():
    return date_entry_hasta.get_date()


def obtenerTipoResenia():
    return combo_tipo_resenia.get()

def obtenerFormatoReporte():
    return combo_formato_reporte.get()
...
fecha_hasta_label = ttk.Label(frame_fh, text="Fecha Hasta", font=fuenta_letras)
...
combo_tipo_resenia.set(valores[0])
...
combo_formato_reporte.set(valores[0])
...
##### --------------- Parametros de ventana y posiciónamiento --------------- ##### 
